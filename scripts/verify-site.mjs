...
assert.equal(urls.length, 12, "Expected homepage, about, contact, project index, and eight cases");
...
for (const url of urls) {
  ...
  assert.ok(doc.querySelector('meta[name="description"]')?.content, `${path}: description`);
  ...
  for (const key of ["og:title", "og:description", "og:image"]) assert.ok(doc.querySelector(`meta[property="${key}"]`)?.content, `${path}: ${key}`);
  assert.equal(
...
for (const [path, doc] of documents) {
  for (const anchor of doc.querySelectorAll("a[href]")) {
    ...
    if (url.hash) assert.ok(target.getElementById(decodeURIComponent(url.hash.slice(1))), `${path}: anchor ${href}`);
  }
}
...
assert.equal(social.status, 200); assert.ok(social.headers.get("content-type").includes("image/png"));
const robots = await (await fetch(`${base}/robots.txt`)).text();
assert.ok(robots.includes("Allow: /")); assert.ok(robots.includes(`${canonicalBase}/sitemap.xml`));
const llms = await (await fetch(`${base}/llms.txt`)).text();
for (const url of urls.filter(url => url.includes("/projects/"))) assert.ok(llms.includes(url));
console.log("PASS all internal links and anchors, 404 responses, portrait, social image, robots, llms.txt");
